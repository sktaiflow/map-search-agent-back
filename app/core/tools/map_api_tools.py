...
class CheckPlanEligibilityTool(StandardizedTool):
    ...
    async def _arun(self, user_id: str, prod_id: str) -> Dict[str, Any]:
        """
        요금제 가입 가능성 조회
        ...
        """
        try:
            endpoint = f"plan/basic-plan_{self.method_api_key}/plan-subscription-previews"
            response = await self.map_client._request(
                "GET", endpoint, 
