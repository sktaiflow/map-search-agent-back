...
async def execute_cypher_query(
    ...
    try:
        ...
        async with neo4j_engine.get_async_session(mode="r") as session:
            """
            cypher transaction으로 날리는 방법 1.
            result = await neo4j_model.execute_read_tx(
                cypher=request.query, params=request.parameters or {}
            )
            """
            # cypher transaction으로 날리는 방법 2.
            result = await neo4j_model.query_database(
                query=request.query,
                params=request.params or {},
            )

            # TODO: 방법 2 에 대한 node parsing 예제 코드 -> 실제 코드는 agent의 postprocess로 실제로 들어가야함
            records = []
        ...
        error_response = CypherResponse(data=[], message=f"Server error: {str(e)}", record_count=0)
