...
class Neo4jSearchRequest(BaseModel):
    ...
    return_type: Optional[int] = Field(
        default=None,
        description="0/2: 상세 응답(포괄 검색 허용), 1: product_id만 반환(정확 검색 우선)",
    )
...
class CypherResponse(BaseModel):
    ...
    data: List[Dict[str, Any]] = Field(
        default_factory=list, description="쿼리 실행 결과 데이터"
    )
    message: Optional[str] = Field(
        default=None, description="추가 메시지 (에러 메시지 등)"
    )
    ...
    case: Optional[str] = Field(
        default=None, description="검색 케이스 (1: 정확매치, 2: 조건완화)"
    )
    cypher: Optional[Union[str, List[str]]] = Field(
        default=None, description="실행된 Cypher 쿼리"
    )
